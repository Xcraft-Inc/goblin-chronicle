...
// FIXME: remove all dependencies on goblin-yennefer (which is not public)
const {
  WorkflowState,
...
class Compendium extends Elf.Alone {
  ...
  loadWorkflowsDebounced = debounce(this.loadWorkflows, 1000);
  loadTriggersDebounced = debounce(this.loadTriggers, 1000);

  async init() {
    const hordeConfig = require('xcraft-core-etc')().load('xcraft-core-horde');

    /* Cases where loading the resources (no horde defined or server side) */
    if (!hordeConfig.hordes?.length || IS_SERVER_SIDE) {
      this.cryo.listenTo(
        'gold',
        async () => await this.loadWorkflowsDebounced()
      );
    }

    this.cryo.listenTo(
      'workflowTrigger',
      async () => await this.loadTriggersDebounced()
    );

    //load now (required for the boot flow dependencies)
  ...
  async onBusinessEventCreated(event, callerDesktopId) {
    ...
    for (const {id, data} of matchedTriggers) {
      await this.trigger(callerDesktopId, id, event.id, data);
    }
  }
  ...
  async _resourcesLoader(namespace) {
    ...
    for (const workflowFolder of workflowFolders) {
      ...
      if (!workflowDef || !resolvedPath) {
        continue; /* removed */
      }
      const hash = await fileChecksum(resolvedPath, {algorithm: 'sha256'});
      const scriptPath = path.join(workflowFolderPath, 'index.js');
      workflowDef.hash = hash;
      workflowDef.sourceName = scriptPath;
  ...
  async _trashDetachedWorkflows(workflows) {
    ...
    const allWorkflows = reader
      .queryArchetype('workflow', WorkflowShape)
      .field('id')
      .all();

    const detachedWorkflows = allWorkflows.filter(
      (id) => !workflowsId.includes(id)
    );

    for (const workflowId of detachedWorkflows) {
      const workflow = await new Workflow(this).create(workflowId, feedId);
      await workflow.remove();
    }

    const allTriggers = reader
      .queryArchetype('workflowTrigger', WorkflowTriggerShape)
      .field('id')
      .all();

    for (const triggerId of allTriggers) {
      const workflowId = `workflow@${triggerId.split('@')[1]}`;
      if (
        !workflowsId.includes(workflowId) &&
        (await WorkflowTriggerLogic.exist(this.cryo, triggerId))
      ) {
  ...
  async loadWorkflows() {
    ...
    for (const {
      ...
      const curHash = await reader
        .queryArchetype('workflow', WorkflowShape)
        .field('hash')
        .where((workflow) => workflow.get('id').eq(id))
        .get();
      if (curHash === hash) {
      ...
      const state = new WorkflowState({
        ...
        useBusinessEvents: useBusinessEvents || false,
        hash,
  ...
  /**
   * Begin a workflow from a one-shot chronomancer entry and clean it up.
   *
   * @param {`system@${string}`} desktopId
   * @param {`workflow@${string}`} workflowId
   * @param {{contextId: `${string}@${string}`, createdBy: `user@${string}`}} meta
   * @param {object} [data]
   * @param {string} [cronEntryName]
   * @returns {Promise<object>} workflow result data object
   */
  async beginScheduledChronicle(
    desktopId,
    workflowId,
    meta,
    data = {},
    ...
    try {
      return await this.beginChronicle(desktopId, workflowId, meta, data);
    } finally {
      if (cronEntryName) {
        const chronomancer = new Chronomancer(this);
        await chronomancer.remove(cronEntryName);
      }
    }
  }

  async beginChronicle(desktopId, workflowId, meta, data, useBusinessEvents) {
