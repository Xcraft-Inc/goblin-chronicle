...
class Compendium extends Elf.Alone {
  ...
  async init() {
    ...
    if (!hordeConfig.hordes?.length || IS_SERVER_SIDE) {
      this.quest.sub(`*::${this.id}.<compendium-gold-updated>`, async () => {
        await this.loadWorkflowsDebounced();
      });

      const cryoAPI = this.quest.getAPI('cryo');
      await cryoAPI.registerLastActionTriggers({
        actorType: 'gold',
        onInsertTopic: `${this.id}.<compendium-gold-updated>`,
        onUpdateTopic: `${this.id}.<compendium-gold-updated>`,
        onDeleteTopic: `${this.id}.<compendium-gold-updated>`,
      });

      this.quest.evt('<compendium-gold-updated>');
    }

    this.quest.sub(
      `*::${this.id}.<compendium-workflowTrigger-updated>`,
      async () => {
        await this.loadTriggersDebounced();
      }
    );

    const cryoAPI = this.quest.getAPI('cryo');
    await cryoAPI.registerLastActionTriggers({
      actorType: 'workflowTrigger',
      onInsertTopic: `${this.id}.<compendium-workflowTrigger-updated>`,
      onUpdateTopic: `${this.id}.<compendium-workflowTrigger-updated>`,
      onDeleteTopic: `${this.id}.<compendium-workflowTrigger-updated>`,
    });

    this.quest.evt('<compendium-workflowTrigger-updated>');
