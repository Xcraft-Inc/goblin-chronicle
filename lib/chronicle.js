...
class Chronicle extends Elf {
  ...
  async getScriptSource(workflowId, name) {
    const namespace = workflowId.split('@')[1];
    const sourceName = `${namespace}.${name}.js`;
    /* Retrieve the chestObjectId according to the sourceName */
    const scripts = await this.cryo.reader(ChestObjectLogic.db);
    const chestObjectId = scripts
      .queryArchetype('chestObject', ChestObjectShape)
      .field('id')
      .where((object) => object.get('name').eq(sourceName))
      .get();
    ...
    /* Retrieve the script file according to the chestObjectId */
    const chest = await new Chest(this);
    const location = await chest.locationTry(chestObjectId);
  ...
  async scriptTask(name) {
    ...
    const script = await this.getScriptSource(workflowId, name);
  ...
  async begin() {
    ...
    const onActivity = (step) => async (elementAPI, executionAPI) => {
      this.log.dbg(`Begin activity.${step}`, elementAPI.type, elementAPI.name);
      ...
      try {
        ...
        this.log.dbg(`End activity.${step}`, elementAPI.type, elementAPI.name);
      }
    };
